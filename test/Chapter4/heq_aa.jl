...
function heq_aa()
    ...
    condref = [1.0, 2.0e2, 1.9e5, 1.9e7, 5.5e9, 6.5e10]
    n = 512
    u0 = ones(n)
    hdata = heqinit(u0, c)
    FS = zeros(n)
    ...
    #
    solok = (norm(itrecords[:, 1], Inf) < 1.0e-7)
    solok || println("Solution error in Anderson H solve")
    histok = (norm(itrecords[:, 2] - fcount, Inf) < 1.0e-5)
    histok || println("History error in Anderson H solve")
    condok = (hresults(itrecords[:, 3], condref) < 1.0e-1)
    condok || println("Condition error in Anderson H solve")
    normok = (hresults(itrecords[:, 4], anormref) < 1.0e-1)
