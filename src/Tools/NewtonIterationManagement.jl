...
"""
function Newtoninit(x0, dx, F!, J!, solver, sham, armmax, armfix, resdec,
       maxit, printerr, pdata, jfact)
    #
    #   Initialize the iteration.
    #
    n = length(x0)
    x = copy(x0)
    ItRules = (dx = dx, f = F!, fp = J!, 
           solver=solver, sham=sham, armmax=armmax, armfix=armfix, 
          resdec=resdec, maxit=maxit, printerr=printerr,
...
errcode = 0
if ~idid
    errcode = NewtonError(resfail, iline, resnorm, toosoon, 
               tol, itc, maxit, armmax, printerr)
end
return (idid, errcode)
end
...
"""
function NewtonClose(x, FS, ithist, stats, 
