...
function Jbvp!(FV, FVP, x, bdata)
    ...
    ndown = diagind(FVP, -1)
    FUP = view(FVP, nup)
    @views FUP[2:2:2n-2] .= zup
    FDOWN = view(FVP, ndown)
    @views FDOWN[1:2:2n-3] .= zdown
end
...
function jacinit!(FVP, n, h2, tv, tvdag, DiagFP)
#
...
#
    for ip = 3:4
        @views FVP[band(ip)] .= 0.0
    end
#
# Put the good bands in the right place.
#
    @views FVP[band(-2)] .= Dm2
    @views FVP[band(-1)] .= Dm1
    @views FVP[band(0)] .= D
    @views FVP[band(1)] .= D1
    @views FVP[band(2)] .= D2
end
